...
<body>
    ...
    <?php
        session_start();
        ob_start();
        if(isset($_SESSION['thongBao']) && $_SESSION['thongBao']==1){
                                ...
                                { 
                                    ...
                                    if (($_FILES["hinhanh"]["type"]=="image/jpeg") ||($_FILES["hinhanh"]["type"]=="image/png") || ($_FILES["hinhanh"]["type"]=="image/jpg") && ($_FILES["hinhanh"]["size"] < 50000) )
                                    {
                                        ...
                                        // {
                                            ...
                                            $tenfilemoi = $_FILES["hinhanh"]["name"];
                                            $tenfilemoi = "./img/products/" . basename($_FILES["hinhanh"]["name"]);
    ...
    <div class="main">
        ...
        <div class="sanpham">
            ...
            <div class="table-footer">
                ...
                <button >
                    <i class="fa fa-plus-square"></i>
                    <a href="./Classes/indexexcel.php" style="color: #fff">Thêm từ Excel</a>
                </button>
                <button >
                    <i class="fa fa-plus-square"></i>
                    <a href="./Classes/img_indexexcel.php" style="color: #fff">Thêm ảnh từ Excel</a>
                </button>
            </div>

            <div id="khungThemSanPham" class="overlay">
                <span class="close" onclick="this.parentElement.style.transform = 'scale(0)';">&times;</span>
                <form method="post" action="admin.php" enctype="multipart/form-data" onsubmit="return themSanPham();">
                    <table class="overlayTable table-outline table-content table-header">
                        ...
                        <tr>
                            <td colspan="2" class="table-footer"> <button type="submit" name="submitThem">THÊM</button> </td>
                        </tr>
                    </table>
        ...
        <div class="donhang">
            <table class="table-header table-striped">
                <tr>
                    ...
                    <th title="Sắp xếp" style="width: 20%" onclick="sortDonHangTable('sanpham')">Chi Tiết Sản phẩm <i class="fa fa-sort"></i></th>
                ...
                <?php   
                    ...
                    $sql = "select hd.MaHD, nd.ho, nd.ten, hd.tongtien, hd.ngaylap, hd.trangthai
                            from hoadon hd, nguoidung nd WHERE hd.MaND = nd.MaND";
                    ...
                    <tr style="color: white; text-align: center;">
                        ...
                        <td><?=  $value['MaHD'] ?></td>
                        <td><?php echo  $value['ho']." ". $value['ten'] ?></td>
                        <td><a class="my_xemchitiet" data-toggle="modal" data-target="#hd<?= $value['MaHD']?>">Xem chi tiết</a></td>
                        <td><?= number_format($value['tongtien']) ?></td>
                        <td><?= $value['ngaylap'] ?></td>
                        <td><?= $value['trangthai'] ?></td>
                        <td class="my_huy"><a  href='./php/deleteDonHang.php?id=<?= $value['MaHD'] ?>' style="color: #fff">Hủy</a></td>
                        <!-- Modal -->
                        <div id="hd<?= $value['MaHD']?>" class="modal fade" role="dialog">
                        <div class="modal-dialog">
                            ...
                            <?php
                                $sqlCTHD ="select sp.TenSP, sp.DonGia, cthd.SoLuong from chitiethoadon cthd, sanpham sp 
                                where cthd.MaSP = sp.MASP and  MaHD = '".$value['MaHD']."'";

                                $resultCTHD = $conn->query($sqlCTHD);
                                $tongTien = 0;
                                foreach ($resultCTHD as $keyCTHD => $valueCTHD) {
                                    $thanhTien = $valueCTHD['DonGia'] * $valueCTHD['SoLuong'];
                                    $tongTien += $thanhTien;
                                ...
                                ?>
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <input type="text" class="form-control" value="<?= $valueCTHD['TenSP'] ?>">
                                    </div>
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text">Đơn giá</span>
                                        </div>
                                        <input type="text" class="form-control" value="<?= number_format($valueCTHD['DonGia']) ?>">
                                    </div>
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text">Số lượng</span>
                                        </div>
                                        <input type="text" class="form-control" value="<?= $valueCTHD['SoLuong'] ?>">
                                    </div>
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text">Thành tiền</span>
                                        </div>
                                        <input type="text" class="form-control" value="<?= number_format($thanhTien) ?>">
                                    </div>
                                ...
                                <div class="input-group mb-3 input-group-lg">
                                    ...
                                    <input type="text" class="form-control" value="<?= number_format($tongTien) ?>">
                                </div>
                            <?php
                            ?>
